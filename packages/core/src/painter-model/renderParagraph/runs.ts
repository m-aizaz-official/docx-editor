 ...
 *
 * `styleRunElement` is the big block — every run-level OOXML property (font,
...
function styleRunElement(
  ...
  if (run.fontFamily) {
    ...
    // matching the same stacks used in textMetrics.ts
 ...
 */
export function applyPmPositions(element: HTMLElement, docFrom?: number, docTo?: number): void {
  if (docFrom !== undefined) {
    element.dataset.docFrom = String(docFrom);
  }
  if (docTo !== undefined) {
    element.dataset.docTo = String(docTo);
  }
}
...
export function paintTextRun(
  ...
  styleRunElement(span, run, resolvedCommentIds);
  ...
  if (run.hyperlink && hyperlinkHref) {
    ...
    // Inherit from the wrapping span (already styled by styleRunElement). Fall
...
function renderInlineImageRun(
  ...
  applyPmPositions(img, run.docFrom, run.docTo);
...
export function paintLineBreakRun(run: LineBreakRun, doc: Document): HTMLElement {
  ...
  applyPmPositions(br, run.docFrom, run.docTo);
 ...
 */
export function paintFieldRun(run: FieldRun, doc: Document, context: RenderContext): HTMLElement {
  ...
  return paintTextRun(resolvedRun, doc, context?.resolvedCommentIds);
}
...
export function paintRun(run: Run, doc: Document, context?: RenderContext): HTMLElement {
  ...
  if (isTabRun(run)) {
    ...
    return paintTabRun(run, doc, 48, undefined); // Default 0.5 inch tab
  }
  if (isImageRun(run)) {
    return paintImageRun(run, doc);
  }
  if (isLineBreakRun(run)) {
    return paintLineBreakRun(run, doc);
  }
